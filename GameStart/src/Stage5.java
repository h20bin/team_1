...
public class Stage5 extends JPanel implements ActionListener, KeyListener {
    ...
    public Stage5 (GameManager manager, int stageNum) {
 ...
 // 유틸리티 메서드: 스프라이트 시트를 로드합니다.
    ...
    private void initializeStage(int stageNum) {
        ...
        try {
            ...
            weaponSprites2 = new SpriteSheet("/Weapon/ball4.png", 72, 108).getFrame(0);
        ...
        for (int i = 0; i < stageNum*25; i++) {
            ...
            switch (i % 2) {
                case 0: // 직선 배열
                    // 직선 배열에서 간격을 좁히고 적의 체력을 증가
                    enemy = new Enemy(50 + (i * 20), -100, 50 + stageNum * 10, enemySprite1, null);
                    break;

                case 1: // 원형 배열
                    // 원형 배열에서 적들이 더 촘촘하게 배치되고 체력이 증가
                    double angle = 2 * Math.PI / (stageNum + 2) * i; // 각도 계산 (적들이 더 많이 배치됨)
                    int centerX = 256;
                    int centerY = -300;
                    int radius = 80; // 반지름 줄이기
                    int x = (int) (centerX + radius * Math.cos(angle));
                    int y = (int) (centerY + radius * Math.sin(angle));
                    enemy = new Enemy(x, y, 50 + stageNum * 10, enemySprite2, null);
                    break;

                default: // 기본: 직선 배열
                    // 기본 배열도 난이도에 맞게 체력 증가
                    enemy = new Enemy(50 + (i * 20), -100, 50 + stageNum * 10, enemySprite2, null);
            }
