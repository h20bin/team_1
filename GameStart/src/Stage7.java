...
public class Stage7 extends JPanel implements ActionListener, KeyListener {
    ...
    public Stage7 (GameManager manager, int stageNum) {
 ...
 // 유틸리티 메서드: 스프라이트 시트를 로드합니다.
    ...
    private void initializeStage(int stageNum) {
        ...
        // 적 초기화
        for (int i = 0; i < stageNum*35; i++) {
            BufferedImage enemySprite1;
            BufferedImage enemySprite2 = null;
            BufferedImage enemySprite3 = null;
            BufferedImage enemySprite4 = null;
            try {
                enemySprite1 = new SpriteSheet("/Weapon/ball2.png", 72, 108).getFrame(0);
                enemySprite2 = new SpriteSheet("/Weapon/ball4.png", 72, 108).getFrame(0);
                enemySprite3 = new SpriteSheet("/Weapon/ball3.png", 72, 108).getFrame(0);
                enemySprite4 = new SpriteSheet("/Weapon/ball1.png", 72, 108).getFrame(0);
            } catch (IOException e) {
                e.printStackTrace();
            ...
            switch (i % 4) {
                case 0: // 직선 배열
                    // 직선 배열 간격을 좁히고, 체력을 증가시키며, 적의 속도를 증가시킴
                    enemy = new Enemy(50 + (i * 20), -100, 50 + stageNum * 20, enemySprite1, null);
                    break;

                case 1: // 지그재그 배열
                    // 지그재그 배열의 간격을 좁히고, 적의 체력을 증가시키며, 이동 패턴을 더 복잡하게 만듦
                    enemy = new Enemy(50 + (i % 2 == 0 ? i * 20 : -i * 20), -100 - (i * 60), 50 + stageNum * 20, enemySprite2, null);
                    break;

                case 2: // 원형 배열
                    // 원형 배열의 반지름을 줄이고, 적의 체력을 증가시키며, 각도를 조금 더 복잡하게 계산
                    double angle = 2 * Math.PI / (stageNum + 2) * i; // 각도 계산
                    int centerX = 256; // 원의 중심
                    int centerY = -300;
                    int radius = 60; // 원의 반지름을 줄여 적들이 더 촘촘하게 배치되도록 함
                    int x = (int) (centerX + radius * Math.cos(angle));
                    int y = (int) (centerY + radius * Math.sin(angle));
                    enemy = new Enemy(x, y, 50 + stageNum * 20, enemySprite3, null);
                    break;

                case 3: // 랜덤 위치
                    // 랜덤 위치의 범위를 늘리고, 적의 체력을 증가시키며, 이동 속도도 증가시킴
                    int randomX = random.nextInt(512 - 40); // 화면 너비 내에서 랜덤 X
                    int randomY = random.nextInt(200) - 300; // 화면 상단 위쪽에서 랜덤 Y
                    enemy = new Enemy(randomX, randomY, 50 + stageNum * 20, enemySprite4, null);
                    break;

                default: // 기본: 직선 배열
                    // 기본 배열도 난이도에 맞게 체력을 증가시키고, 간격을 좁힘
                    enemy = new Enemy(50 + (i * 20), -100, 50 + stageNum * 20, enemySprite1, null);
            }

            // 무기를 강화 (적의 공격력과 속도 증가)
            Weapon enemyWeapon = new Weapon(enemy, weaponSprites1, 3 + stageNum * 2, 5 + stageNum * 3, 2, bulletFrames, 6);
