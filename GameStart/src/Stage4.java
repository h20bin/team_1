...
public class Stage4 extends JPanel implements ActionListener, KeyListener {
    ...
    public Stage4 (GameManager manager, int stageNum) {
 ...
 // 유틸리티 메서드: 스프라이트 시트를 로드합니다.
    ...
    private void initializeStage(int stageNum) {
        ...
        try {
            weaponSprites1 = new SpriteSheet("/Weapon/ball3.png", 72, 108).getFrame(0);
            weaponSprites2 = new SpriteSheet("/Weapon/ball2.png", 72, 108).getFrame(0);
            weaponSprites3 = new SpriteSheet("/Weapon/ball1.png", 72, 108).getFrame(0);
        ...
        for (int i = 0; i < stageNum*20; i++) {
            ...
            switch (i % 4) {
                case 0: // 직선 배열
                    // 배치 간격을 좁히고 체력 증가
                    enemy = new Enemy(50 + (i * 20), -100, 50 + stageNum * 10, enemySprite4, null);
                    break;

                case 1: // 지그재그 배열
                    // 더 복잡한 지그재그 배치
                    enemy = new Enemy(50 + (i % 2 == 0 ? i * 30 : -i * 30), -100 - (i * 60), 50 + stageNum * 10, enemySprite1, null);
                    break;

                case 2: // 원형 배열
                    // 원형 배열에 적의 수를 늘리고 반지름을 줄여 적들이 더 촘촘해지게 함
                    double angle = 2 * Math.PI / (stageNum + 4) * i; // 각도 계산 (적들이 더 많이 배치됨)
                    int centerX = 256;
                    int centerY = -300;
                    int radius = 80; // 반지름 줄이기
                    int x = (int) (centerX + radius * Math.cos(angle));
                    int y = (int) (centerY + radius * Math.sin(angle));
                    enemy = new Enemy(x, y, 50 + stageNum * 5, enemySprite2, null);
                    break;

                case 3: // 랜덤 위치
                    // 랜덤 배치 위치를 더 다양화하고 체력을 증가시킴
                    int randomX = random.nextInt(512 - 40);
                    int randomY = random.nextInt(200) - 300;
                    enemy = new Enemy(randomX, randomY, 50 + stageNum * 10, enemySprite3, null);
                    break;
